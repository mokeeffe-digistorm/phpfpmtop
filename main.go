	...
	for {
		...
		case t := <-timer.C:
			...
			for _, pro := range s.Processes {
				...
				// Don't list out requests to the PHP-FPM status endpoint generated by phpfpmtop
				if strings.Contains(pro.RequestURI, "(phpfpmtop)") {
					continue
				}
				...
				fmt.Printf("%s %.*s %s\033[K", part, uriWidth, pro.RequestURI, pro.Script)

				// Reset ANSI colors etc.
				fmt.Printf("\033[0m")
			}
